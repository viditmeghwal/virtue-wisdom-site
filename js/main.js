...
if (nav) {
  const onScroll = () => {
    ...
    if (nav.dataset.heroDark === 'true') {
      const hero = document.querySelector('.hero-cine, .hero-bb, .hero');
      if (hero) {
...
// ============================================
//  CONVERSION MODULE — slide-in panel + sticky bar
//  Triggers on scroll depth, dismissible, session-persisted.
// ============================================
(function() {
  // Trigger thresholds (% of page scrolled)
  const SLIDE_IN_THRESHOLD = 0.42; // 42% — fires after the user is invested in the read
  const STICKY_BAR_THRESHOLD = 0.18; // 18% — fires earlier on sector pages (intent-stage)

  function setupTrigger(el, threshold, dismissKey) {
    if (!el) return;
    // Honour prior-session dismissal
    ...
    function check() {
      ...
      if (progress >= threshold) {
        ...
        // GA4 — track that the conversion module displayed
        if (typeof gtag === 'function') {
          gtag('event', 'cta_displayed', {
            cta_type: el.classList.contains('slidein-cta') ? 'slide_in' : 'sticky_bar',
    ...
    // Initial check in case the page is already past the threshold
    setTimeout(check, 600);

    // Wire dismiss button
    const closeBtn = el.querySelector('[data-cta-close], .slidein-cta__close, .sticky-cta-bar__close');
    if (closeBtn) {
  ...
  setupTrigger(
    document.getElementById('slideinCta'),
    SLIDE_IN_THRESHOLD,
    'vw_slidein_dismissed'
  );
  setupTrigger(
    document.getElementById('stickyCta'),
    STICKY_BAR_THRESHOLD,
    'vw_stickybar_dismissed'
  );

  // Track inline-CTA clicks too (no scroll trigger needed — they're in flow)
  document.querySelectorAll('.inline-cta__btn').forEach(btn => {
    btn.addEventListener('click', () => {
      if (typeof gtag === 'function') {
        gtag('event', 'cta_click', {
